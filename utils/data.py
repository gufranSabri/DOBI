...
class SmolTalkProcessor(DataProcessor):
    ...
    def line2data(self, indexed_example: tuple) -> list:
        ...
        raw_fraction = float(getattr(self.config, "RAW_TEXT_FRACTION", 0.0))
        ...
        for idx in range(1, len(messages)):
            ...
            # A fraction of examples is rendered as PLAIN TEXT (no chat template):
            # lm-eval benchmarks send raw completion-style prompts, and a readout
            # trained only on chat-formatted inputs is out-of-distribution there.
            # `random` is seeded in set_rng_state, so this split is reproducible and
            # identical across DDP ranks.
            if raw_fraction > 0.0 and random.random() < raw_fraction:
            ...
            # The trainer (FlowTrainer) compares pred_logits[t] / pred_hs[t] against
            # position t directly and does NOT shift internally — so this pre-shift
            # is what aligns targets to predictions. The leading [1:] drops one
            # context position so that the last context token (the generation prompt)
            # is the first scored position, predicting the first response token.
